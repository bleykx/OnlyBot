    ...
    initializeTypeaheadForScripts: function (inputId, scriptNames) {
        ...
        if (typeof Bloodhound !== 'undefined') {
            ...
            $input.typeahead(
                ...
                await DotNet.invokeMethodAsync('OnlyBot_Client', 'UpdateScriptSearchQuery', suggestion);
            });
        } else {
            console.error("Bloodhound is not defined.");
        }
    },
    initializeTypeaheadForProxies: function (inputId, proxiesNames) {
        // Vérifiez que Bloodhound est défini
        if (typeof Bloodhound !== 'undefined') {
            var scripts = new Bloodhound({
                datumTokenizer: function (datum) {
            ...
            $input.typeahead(
                ...
                {
                    name: 'scripts',
                    source: substringMatcher(proxiesNames),
                    limit: 10, // Limiter le nombre de suggestions à 10
                    templates: {
                        suggestion: function (data) {
                            // Highlight the matching part of the text
                ...
                $(this).typeahead('val', suggestion);
                await DotNet.invokeMethodAsync('OnlyBot_Client', 'UpdateProxySearchQuery', suggestion);
            });
        } else {
            console.error("Bloodhound is not defined.");
        }
